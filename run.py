...
def main():
    ...
    #read input data
    train_dt, test_dt, word2id, id2word = utils.read_data(opt.input_train, opt.input_test) # 마찬가지로 gen_data에서 만든 train과 test데이터를 불러옴
    #save the generated word vocab
    with open('dict/word2id.json', 'w+') as f: # w+ => 파일이 없으면 새로 만든다. word2id.json 이름으로 저장
        f.write(json.dumps(word2id))
    with open('dict/id2word.json', 'w+') as f: # id2word.json 이름으로 저장
        f.write(json.dumps(id2word))
    print('-------------DATA----------------', len(train_dt), len(test_dt))
    if opt.PLOT_FIG:
        plot_train, plot_eval, plot_acc, plot_ret1, plot_ret2, \
        plot_test1, plot_test2 = [], [], [], [], [], [], [] # list 생성
    ...
    random.shuffle(train_dt) #train_dt에 있는 data를 random하게 섞음

    #generate positive and negative examples from the test data
    #test data를 이용하여 random하게 positive와 negative example을 생성
    ...
    for dt_size in data_size: #데이터 수 1/10부터 1까지
        data = train_dt[:dt_size]
        print()
        print('DATA SIZE:', dt_size)
        random.shuffle(data)
        if opt.k_fold: #cross validation #False
